...
echo "<select name='Estado' class='campos' id='estado' onChange='cargaContenido(this.id)'>";
...
if (sdafs){
$qry_select_mun= "SELECT id_municipio, cve_estado, Municipio FROM municipios WHERE cve_estado='.$id.' ORDER BY Municipio";
$rs = $db->Execute($qry_select_mun) or die("Error: $qry_select.<br />" . $db->ErrorMsg());

while (!$rs->EOF) {
		
		$id= $rs->fields['id_municipio'];
		$opcion= $rs->fields['Municipio'];	
		echo "<option value='". $id ."'>". $opcion ."</option>";
		$rs->MoveNext();
		}
	echo "</select>";
...
function generaMunicipio()
{
	include ('adodb/adodb.inc.php');
	include "exodus.inc.php";

echo "<select name='municipio' class='campos' id='municipio' onChange='cargaContenido(this.id)'>";
echo "<option value='0'>Elige</option>";

$qry_select_mun= "SELECT  id_municipio, Municipio FROM municipios ORDER BY Municipio";
$rs_mun = $db->Execute($qry_select) or die("Error: $qry_select.<br />" . $db->ErrorMsg());
